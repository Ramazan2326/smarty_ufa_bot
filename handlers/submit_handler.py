...
async def process_q4(msg: types.Message, state: FSMContext):
    submits = service.spreadsheets().values().get(
        spreadsheetId=spreadsheet_id,
        range='Заявки!A:I',
        majorDimension='ROWS'
    ).execute()
    submits = submits['values'][2:]
    if msg.text == 'нет' or msg.text == 'Нет' or msg.text == 'НЕТ':
        ...
        photo = msg.photo[-1]
        try:
            file_info = await mybot.get_file(photo.file_id)
            downloaded_photo = await mybot.download_file(file_info.file_path)
            with open(f'dynamic/photos/{int(submits[-1][0]) + 1}.jpg',
                      'wb') as file:
                file.write(downloaded_photo.read())

            file_path = f'dynamic/photos/{int(submits[-1][0]) + 1}.jpg'
            file_title = f'{int(submits[-1][0]) + 1}.jpg'
            ...
            await state.update_data(QUESTION_4=downloaded_photo)
            await state.set_state(SubmitFormStates.QUESTION_5)
            await msg.answer(
                '5. Оставь свой контактный номер телефона (пример 89999999999)'
            )
