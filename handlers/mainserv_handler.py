...
builder_commands_servicemens_employee = InlineKeyboardBuilder()
employees_buts = []
for i in range(len(servicemen_values)):
    employees_buts.append(builder_commands_servicemens_employee.button(
        text=f"{servicemen_values[i][1]} {servicemen_values[i][2][0]}."
             f" – {servicemen_values[i][4]}",
        callback_data=CommandsCallback(foo=f"/{servicemen_values[i][0]}")
    ))
builder_commands_servicemens_employee.adjust(1)
...
async def process_red_q1(msg: types.Message, state: FSMContext):
    try:
        ...
        await msg.answer(
            "Нужно было вводить фамилию сервисмена. Введите снова.")
        await state.set_state(RedirectFormStates.QUESTION_1)
